...
**引数:** 機能名 (例: `/add-feature Google Drive統合機能`)
...
実装開始前に、適切なモデルを選択してください。詳細は `docs/development-guidelines.md#25-モデル選択のベストプラクティス` を参照。

**v3.0実装計画での推奨**:
- **Phase 1（マルチテナント基盤）**: Opus推奨
  - Firebase統合、認証実装、GCS構造変更 → Opus
  - チーム管理API後半、フロントエンド統合 → Sonnet
- **Phase 2（コア検索強化）**: Opus → Sonnet
  - FR-112, FR-116のロジック実装 → Opus
  - UI実装、テスト → Sonnet
- **Phase 3（ノート管理）**: Sonnet（Sudachi統合のみOpus）
- **Phase 4（UI/UX調整）**: Sonnet
- **Phase 5（テスト・デプロイ）**: Sonnet

**判断基準**:
- 新技術・複雑なアルゴリズム・アーキテクチャ判断 → **Opus**
- 既存パターン適用・CRUD・UI実装 → **Sonnet**
- タイポ修正・小さなバグ修正 → **Haiku**
...
1. Grepツールを使用し、機能名に関連するキーワードでソースコード(`frontend/`, `backend/`)を検索する。
  ```bash
  Grep('[機能に関連するキーワード]', 'frontend/')
  Grep('[機能に関連するキーワード]', 'backend/')
  ```
